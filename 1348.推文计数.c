/*
 * @lc app=leetcode.cn id=1348 lang=c
 *
 * [1348] 推文计数
 *
 * https://leetcode-cn.com/problems/tweet-counts-per-frequency/description/
 *
 * algorithms
 * Medium (27.03%)
 * Likes:    17
 ...
 * [startTime + delta*1, startTime + delta*2>, [startTime + delta*2, startTime
 * + delta*3>, ... , [startTime + delta*i, min(startTime + delta*(i+1), endTime
 * + 1)>，其中 i 和 delta（取决于 freq）都是非负整数。
 * 
 * 
 * 
 * 
 ...
 * tweetCounts.getTweetCountsPerFrequency("minute", "tweet3", 0, 59); // 返回
 * [2]。统计频率是每分钟（60 秒），因此只有一个有效时间间隔 [0,60> - > 2 条推文。
 * tweetCounts.getTweetCountsPerFrequency("minute", "tweet3", 0, 60); // 返回
 * [2,1]。统计频率是每分钟（60 秒），因此有两个有效时间间隔 1) [0,60> - > 2 条推文，和 2) [60,61> - > 1
 * 条推文。 
 * tweetCounts.recordTweet("tweet3", 120);                            //
 * "tweet3" 发布推文的时间分别是 0, 10, 60 和 120 。
 * tweetCounts.getTweetCountsPerFrequency("hour", "tweet3", 0, 210);  // 返回
 * [4]。统计频率是每小时（3600 秒），因此只有一个有效时间间隔 [0,211> - > 4 条推文。
 * 
 * 
 * 
 * 
