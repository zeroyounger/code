/*
 * @lc app=leetcode.cn id=736 lang=c
 *
 * [736] Lisp 语法解析
 *
 ...
 * add 语法表示为 (add e1 e2)，其中 add 语法总是以字符串 "add"来表示，该语法总是有两个表达式e1、e2, 该语法的最终结果是
 * e1 表达式的值与 e2 表达式的值之和。
 * mult 语法表示为 (mult e1 e2) ，其中 mult 语法总是以字符串"mult"表示， 该语法总是有两个表达式
